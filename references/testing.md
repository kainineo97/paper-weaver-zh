...
普通编辑器与 VIP HTML 导入分别记载，不能用一种路径的成功替代另一种。用户指定普通编辑器时，不打开“导入 HTML 代码”：逐项核对标题与摘要、正文首尾和章节数、段落空白、图片在对应图注前的顺序及加载、图注样式，并确认私有草稿保存反馈或标注不确定性。秀米图库上传数量不等于正文实际插图数量。若页面清洗富文本样式，使用普通控件恢复，不能把本地预览截图当作秀米验收结果。

005 Tcell to MK 的非 HTML 路线曾核对 7 个编号标题、17 张正文图片与 17 条图注，图注为 16px 灰色；测试登录账号显示 VIP，但未使用 VIP HTML 导入。这个案例验证普通编辑器的操作路径，不证明每个免费账号都具有同样的额度或权限。另一篇文章应按自身内容清单验收，而不是硬套上述数量与色值。VIP 路线另检查导入前后 CSS、`data:image/...` 图片与保存后的实际页面。不得为 skill 单元测试擅自上传用户稿件或修改在线草稿。
