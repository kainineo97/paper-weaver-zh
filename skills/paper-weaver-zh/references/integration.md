...
以下 `SKILL` 表示当前 skill 的实际绝对路径，`SKILLS` 表示依赖所在 skills 目录；命令中的路径均需替换。`--skills-root` 可重复；依赖查找顺序为显式目录、本 fork 的仓库内依赖、已安装依赖。仓库根保留 ReoNa 的 `SKILL.md` 和 `scripts/`，`skills/` 放置其余独立 skill；从仓库运行不依赖开发机的全局安装。
...
代理读取论文、写分析与三阶段稿件、完成科学复核，并定稿标题、摘要及全部图片之后，再运行 `fingerprint`，把结果填入 `analysis/review.yaml`。此命令只输出当前摘要，不生成“已审核”结论。`check` 会检查审核记录是否绑定当前稿件；它不能判断科研结论是否真实。
...
本 skill 是组合层；不把 ReoNa 上游项目或第三方写作规则说成用户原创。此 fork 将依赖按独立目录随包提供，不在组合脚本中再复制 renderer 或润色词典。来源和许可证见仓库根的 `THIRD_PARTY_NOTICES.md`；普通写作不触发仓库、打包或发布操作，用户另行请求时按其指定范围执行。
