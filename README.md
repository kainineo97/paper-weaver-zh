...
普通编辑器路线已经用“005 Tcell to MK”实操验证：未触碰 HTML 代码导入；在编辑器中完成正文与空行、7 个编号标题、17 张插图和 17 条图注，最终审计图注均为 16px 灰色。**测试时登录账号显示 VIP，但实际使用的是普通编辑器模式**；因此这是对非 HTML 工作流的验证，不是对所有免费账号权限或额度的独立保证。不同文章的图片与标题数量按其稿件清单核对，不沿用示例数值。
