...
flowchart LR
    ...
    H --> I{需要秀米吗}
    I -->|VIP| J[HTML 导入预览<br/>检查样式与图片后保存草稿]
    I -->|普通账号| K[富文本 / Word / Markdown<br/>必要时手动恢复样式]
    I -->|不需要| L[本地交付]
```
...
| 交付层 | `article.md`、`dist/`、可选 `xiumi-import.html` | 生成本地文章和页面；第三方草稿保存与公开发布保持独立授权 |
...
秀米是本地构建后的可选分支，不是默认发布目标。只有用户明确选择秀米时才准备 `dist/xiumi-import.html` 或相应回退材料。

| 账号状态 | 推荐路径 | 边界 |
|---|---|---|
| VIP | “更多 → 导入 HTML 代码”，预览后确认导入并保存草稿 | 秀米仍可能清洗 CSS 或清除 `data:image/...` 图片 |
| 普通账号 | 复制富文本，或导入 Word/Markdown/公众号文章；必要时使用秀米原生组件恢复样式 | 不承诺存在 HTML 导入入口 |
| 未登录或等级不确定 | 先交付本地阅读页和复制页，登录后依据页面实际入口选择 | 不索要密码、验证码或长期授权 |

VIP 导入前后都要检查紫色标题、灰色图注、段落间距、图片数量和实际加载状态。最终导入和保存会修改第三方草稿，提交前需要用户确认；公开发布、公众号同步和群发需要另行授权。详细流程见 [`references/xiumi-import.md`](references/xiumi-import.md)。
